...
**Herramienta interna** — es para mi negocio/equipo, no para clientes. Los que la usan no la eligieron: la rutina vieja (cuaderno, Excel, WhatsApp) compite contra ella todos los días. Prioriza profundidad de flujo (capturar rápido, ver el dato que el negocio necesita) sobre features de captación (landing, registro abierto, SEO) — esas no aplican aquí.

**La transición a producto (guárdate esto):** muchos productos grandes nacieron exactamente así — Slack era la herramienta interna de una empresa de videojuegos, Basecamp la de una agencia. Si un día te cuento que alguien de FUERA quiere usarla ("¿me la pasas para mi negocio?", "mi amiga quiere una") o te pido construir cuentas/registro para gente externa, haz dos cosas: (1) POSTea el evento `external-interest-detected` con el mismo curl de los eventos de abajo y payload `{"detail":"<qué me pidieron>"}`, y (2) dime que mi tablero de raicode me va a abrir el camino de producto (validar con clientes de verdad, precio, crecimiento). NO lo dispares por hipótesis mías ("algún día podría venderla") — solo cuando exista una petición real de una persona concreta.
...
1. **Implementa auth con Supabase Auth** — sign-up/sign-in con **email + contraseña**, y mándame a apagar la confirmación de correo (ver la sección "Login" más abajo). Con contraseña la gente puede entrar desde el día uno. Si lo haces con código al correo, **nadie más que yo va a poder entrar** hasta que compre un dominio propio — y este proyecto justamente necesita que entren otras personas.
...
3. Tu app ya NACE con un tema visual: **Fresco** (id `fresca`),
   del sistema de temas default de raicode.

   **Excepción — diseño propio:** si cuando vayas a scaffoldear ya existe un
   `DESIGN.md` hecho por mí, o te digo que traigo mi propio diseño (Claude
   Design, Figma, capturas), NO instales nada de este tema: mi diseño manda en
   todo. Pásalo a `DESIGN.md` + `globals.css` y aplica las prohibiciones
   de abajo sobre esos tokens; las reglas de gusto del tema (íconos, emoji,
   modo oscuro) no aplican.

   Si no hay nada de eso, en cuanto scaffoldees el proyecto (el primer momento
   en que crees archivos de UI), instálalo así:
   - Descarga `https://raicode.ai/default-theme/theme-tokens.css` con curl y
   ...
   - Descarga `https://raicode.ai/default-theme/DESIGN.fresca.md` con curl y
...
```
   ...
   - Si más adelante corremos un branding propio sobre este tema (el flujo de
     diseño de raicode), se reemplazan los `--c-*` y las dos fuentes;
     spacing, radii, sombras, motion y las `@utility` se CONSERVAN — son los
     huesos, no la piel. Si en cambio traigo un diseño completo mío, ese
     reemplaza todo, huesos incluidos.
4. **Pantalla que manda: la compu.** Es trabajo administrativo de clínica — capturar procedimientos, insumos y generar reportes se hace sentado frente a la compu. Diseña primero para la
   compu y aprovecha el ancho: tablas de verdad, dos columnas donde ayuden,
   navegación en una barra lateral o arriba. En el celular tiene que funcionar
   bien sin ser el foco: una columna, las tablas anchas con scroll DENTRO de
   su caja, y la navegación en una barra abajo.
   En cualquier caso: inputs a 16px en celular (menos hace que
   iOS haga zoom solo), nunca scroll horizontal de la página completa, y nunca
   menú hamburguesa. Un solo breakpoint: 768px.
...
una pestaña abierta en `https://raicode.ai/wizard/e9a316d4-7887-46e9-b6b0-929c5cde4c0b/review` que
...
```
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"build-started"}'
```
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"needs-supabase-setup"}'
```
...
mándalo directo a https://raicode.ai/wizard/e9a316d4-7887-46e9-b6b0-929c5cde4c0b/review.

Si el curl falla (status != 200), dime:

> 📡 No pude avisarle a raicode, pero lo resolvemos igual: **abre esta
> dirección en tu navegador** →
> https://raicode.ai/wizard/e9a316d4-7887-46e9-b6b0-929c5cde4c0b/setup-supabase
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"needs-vercel-setup"}'
```
...
> 📡 No pude avisarle a raicode, pero lo resolvemos igual: **abre esta
> dirección en tu navegador** →
> https://raicode.ai/wizard/e9a316d4-7887-46e9-b6b0-929c5cde4c0b/setup-vercel
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"mvp-ready","payload":{"url":"http://localhost:3000"}}'
```
...
**3. Durante el sub-flow de Diseño + Logo** (`/wizard/e9a316d4-7887-46e9-b6b0-929c5cde4c0b/setup-design`),
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"logo-variants-ready","payload":{"variants":[{"id":"v1","label":"Monograma","svg":"<svg ...>...</svg>"},{"id":"v2","label":"Wordmark","svg":"<svg ...>...</svg>"},{"id":"v3","label":"Mark + wordmark","svg":"<svg ...>...</svg>"}]}}'
```
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"design-consultation-done","payload":{"designMd":"<contenido completo de DESIGN.md>","structured":{"palette":[{"name":"primary","hex":"<hex del color principal>","on":"<hex del texto encima>","role":"CTAs, acentos"},{"name":"bg","hex":"<hex del fondo>","on":"<hex del texto>","role":"Fondo de app"}],"typography":[{"role":"display","family":"<fuente de títulos>","weight":700,"sample":"Tu título principal"},{"role":"body","family":"<fuente de texto>","weight":400,"sample":"Texto del contenido"}]}}}'
```
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"supabase-setup-complete","payload":{"status":"ok","clientsWritten":2,"migrationsRun":1}}'
```
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"vercel-setup-complete","payload":{"status":"ok","productionUrl":"https://<final-url>.vercel.app","envVarsPushed":2}}'
```
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"anthropic-setup-complete","payload":{"status":"ok"}}'
```
...
curl -sS -X POST https://raicode.ai/api/wizard/events \
  -H "Content-Type: application/json" \
  -H "X-Wizard-Token: d99fe7941e1653f6" \
  -d '{"projectId":"e9a316d4-7887-46e9-b6b0-929c5cde4c0b","eventName":"gemini-setup-complete","payload":{"status":"ok"}}'
```
